# Stack Tecnológico del Proyecto Ciompi

Aplicación web para la gestión de cobranzas y financiamiento de vehículos: clientes, empresas, vehículos, financiamientos, cuotas y operaciones.
...
| **SQL (MongoDB)** | No aplica — base de datos NoSQL |
...
| **Mongoose** | 8.18.1 | ODM para modelado de datos y conexión |
| **mongodb** | 7.0.0 | Driver nativo de MongoDB (usado en scripts de migración) |
...
| Script | Descripción |
| --- | --- |
| `resetUsuarios` | Reinicia el usuario administrador |
| `testConnection` | Prueba la conexión a MongoDB |
| `migrar-xml`, `migrar` (v2, etc.) | Migración de datos históricos |
| `migracion-v3-fechas-reales`, `migracion-mejorada` | Migraciones v3 |
| `ver-ejemplos-xml`, `validar-xml` | Inspección y validación de XML |

Los scripts de migración usan **xml2js** (0.6.2) para parsear los datos XML históricos y **MongoClient** para insertarlos en MongoDB.
...
| Archivo | Descripción |
| --- | --- |
| `start.js` | Script de inicio que ejecuta `pnpm start` (con `cwd: C:\ciompi`) |
| `iniciador.bat`, `iniProyect.bat` | Scripts de arranque en Windows |
| `pm2 nuevo.txt` | Notas de proceso con PM2 para mantener la app en ejecución |
| `next.config.ts` | Configuración de Next.js (actualmente vacía) |
...
pnpm test-db    # prueba de conexión a BD
pnpm migrar     # migración v3
pnpm validar-xml / ver-xml   # utilidades XML
```
...
Scripts:   Node.js · xml2js · MongoClient
